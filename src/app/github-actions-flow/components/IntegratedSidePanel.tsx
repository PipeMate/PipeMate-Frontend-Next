        ...
        <Tabs defaultValue="fields">
          ...
          <TabsContent value="json">
            <div className="space-y-2">
              ...
              />
              {configError && (
                <div className="text-xs text-red-500">{configError}</div>
              )}
            </div>
          </TabsContent>
        </Tabs>
...
export const IntegratedSidePanel: React.FC<IntegratedSidePanelProps> = ({
  ...
  const convertAreaNodeToServerBlock = useCallback(
    (node: AreaNodeData): ServerBlock => {
      return {
                  ...
                  <CardContent>
                    <div className="space-y-3">
                      <p className="text-sm text-gray-600">
                        GitHub Secrets를 생성하고 관리하세요. 워크플로우에서
                        사용되는 secrets가 누락된 경우 여기서 추가할 수
                        있습니다.
                      </p>
                      <GithubTokenDialog
                        trigger={
