...
export const useDragDrop = () => {
  ...
  const [touchStartPos, setTouchStartPos] = useState<{ x: number; y: number } | null>(
    null,
  );
  ...
  const handleKeyNavigation = useCallback(
    (e: React.KeyboardEvent, currentNodeId: string, nodes: AreaNodeData[]) => {
   ...
   * 블록을 노드 데이터로 변환
   */
  const convertBlockToNodeData = useCallback((block: ServerBlock): WorkflowNodeData => {
    return {
      label: block.name,
      type:
        block.type === 'trigger'
